...
- 主环境依赖安装
- WSL 中的系统包安装
- WSL 中 legacy detector 依赖安装
- TensorFlow / 相关运行库下载与解压

所以如果安装阶段长时间有输出，不一定是卡死，很多时候只是依赖仍在正常安装。

另外，安装脚本现在已经**默认内置国内镜像源配置**，用于减少 WSL 内首次安装时访问官方源失败的问题：

- WSL 的 Ubuntu APT 源默认走 **清华源**
- WSL 中的 `pip` 默认走 **清华 PyPI 镜像**

也就是说，正常情况下你不需要再额外手动给安装脚本传清华源参数；脚本默认就会优先使用这些镜像。
...
- 第一次安装 Ubuntu 时，需要你先完成 Ubuntu 的用户名 / 密码创建流程
- **完成 Ubuntu 首次初始化后，需要重新运行一次安装脚本**，因为第一次安装 Ubuntu 时脚本不会自动继续往下执行

如果脚本提示你先运行一次：

- `wsl -d Ubuntu`

那就先完成 Ubuntu 首次初始化，然后再回到项目里**重新运行**安装脚本。
